...
The model was trained for 50 epochs: we got the best results at the 32th epoch. The training loss and the validation mAP are shown in the following figure \ref{fig:loss}.
...
To evaluate the model we used the Mean Average Precision (mAP) metric for both the Bounding Box and the Segmentation Mask. In particular, both the problems needs a threshold in order to consider the prediction as correct. We used a threshold of 0.7 for both the tasks.

Furthermore, we chose to track only the mask and bounding box loss and mAP. We specifically decided not to keep track of the classification loss since the model has to predict only one class and the task is not the main focus of the project. It's worth noting that it's been optimized anyway.


The model, at his better epoch, performed with an mAP of 50\% for the Segmentation Mask and 55\% for the Bounding Box. It's important to note that the chosen model is not the best one in terms of Bounding Box mAP obtained during all the training process: since the task is more focused on the segmentation task, we chose the model that performed best in this specific case. Furthermore, the mAP difference between the best model (Bounding Box) and the chosen one is negligible.
...
It's also worth noting that, as shown in figure \ref{fig:nms}, the model lacks the Non-Maximum Suppression (NMS) algorithm. This is the reason why the model predicts multiple overlapping bounding box in the same pothole. The ambiguity is due to the diversity in depth of the pothole: the model knows the concept of depth and tries to predict the pothole in different depth levels.

Furthermore, since the mAP evaluate all the Bounding Box present in the image (even the overlapping ones that doesn't cover all the ground truth area) the lacks of NMS lowers the performance of the model.
This can be solved in future works. 
...
\begin{figure}[ht]
    ...
    \caption{Example of multiple overlapping bounding box in the same pothole.}
    \label{fig:nms}
\end{figure}
